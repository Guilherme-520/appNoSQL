...
class DBHandler // creating a constructor for our database handler.
    ...
    fun updateCourse(courseId: Int, courseName: String, courseDuration: String, courseDescription: String, courseTracks: String): Int {
        val db = this.writableDatabase
        val values = ContentValues()
        values.put(NAME_COL, courseName)
        values.put(DURATION_COL, courseDuration)
        ...
        return db.update(TABLE_NAME, values, "$ID_COL=?", arrayOf(courseId.toString()))
    }


    fun deleteCourse(courseId: Int): Boolean {
        val db = this.writableDatabase
        return db.delete(TABLE_NAME, "$ID_COL=?", arrayOf(courseId.toString())) > 0
    }
